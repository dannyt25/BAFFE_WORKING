...
f0min     = [50]    ;     % Min F0 (Hz)                               [50]
f0max     = [500]   ;     % Max F0 (Hz)                               [500]
tframe    = [0.01]  ;     % frame size (s)                            [0.01]
tlpw      = [0.005] ;     % low pass filter window size (s)           [0.005]
tcorw     = [0.0256];     % correlation window size (s)               [0.0075]
candtr    = [0.3]   ;     % minimum peak in NCCF                      [0.3]
lagwt     = [0.3]   ;     % linear lag taper factor                   [0.3]
freqwt    = [0.02]  ;     % cost factor for F0 change                 [0.02]
vtranc    = [0.005] ;     % fixed voice-state transition cost         [0.005]
vtrac     = [0.5]   ;     % delta amplitude modulated transition cost [0.5]
vtrsc     = [0.5]   ;     % delta spectrum modulated transition cost  [0.5]
vobias    = [0.0]   ;     % bias to encourage voiced hypotheses       [0.0]
doublec   = [0.35]  ;     % cost of exact doubling or halving         [0.35]
...
ncands    = [20]    ;     % max hypotheses at each frame              [20]
trms      = [0.03]  ;     % window length for rms measurement         [0.03]
dtrms     = [0.02]  ;     % window spacing for rms measurement        [0.02]
preemph   = [-7000] ;     % s-plane position of preemphasis zero      [-7000]
nfullag   = [7]     ;     % number of full lags to try (must be odd)  [7]
