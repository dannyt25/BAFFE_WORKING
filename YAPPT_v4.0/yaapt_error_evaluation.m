...
prompt = 'Perform on Clear Voice or SRAEN? 0 or 1 Respectivly \n';
SRAEN = input(prompt);
...
for k=1:10    
    ...
    for n = 1:SNR_LENGTH
        ...
        SNR = SNRZ(n);
        if( SRAEN==1 )
            Sig = filtfilt(fir1(150,[300 3400].*2./fs),1,Sig);
        end
...
for m = 1:SNR_LENGTH
    m
    [GROSS_5_YAAPT(m),~,~]        = Gerr_DT( PITCH_REF, PITCH_EST{m,1}, 0.05 );
    [GROSS_20_YAAPT(m),~,FINE_YAAPT(m)] = Gerr_DT( PITCH_REF, PITCH_EST{m,1}, 0.2 );
end

clearvars -except GROSS_5_YAAPT GROSS_20_YAAPT FINE_YAAPT

% save ('../BAFFE/V3/yaapt_error_eval_clear.mat')
% save ('../BAFFE/V3/yaapt_error_eval_phone.mat')
